...
export default function ExamResults() {
  ...
  // Trigger animation on mount & fetch results
  useEffect(() => {
    const fetchResults = async () => {
      if (!user || !user._id || !examId) {
        setLoading(false);
        return;
      }
      try {
        const submission = await submissionService.getUserSubmission(examId, user._id);
        const examData = submission.examId;
        ...
        setResult({
          ...
          wrongAnswers: submission.totalQuestions - submission.score,
          skippedQuestions: 0,
          totalPoints: submission.totalQuestions, // assuming 1pt each
          earnedPoints: submission.score,
          percentage: submission.percentage,
          passingScore: 50,
          passed: submission.percentage >= 50,
          timeTaken: submission.timeTaken,
          timeLimit: examData.timeLimitMinutes,
          completedAt: submission.submitTime,
        });
      } catch (err) {
  ...
  const getPerformanceMessage = () => {
    if (!result) return { title: '', message: '', type: 'fail' };
